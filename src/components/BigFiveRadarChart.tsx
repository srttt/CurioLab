import type { DimensionScore } from "@/types/assessment";

function polarPoint(center: number, radius: number, index: number, total: number) {
  const angle = -Math.PI / 2 + (index / total) * Math.PI * 2;

  return {
    x: center + Math.cos(angle) * radius,
    y: center + Math.sin(angle) * radius
  };
}

function pointsFor(scores: DimensionScore[], center: number, radius: number) {
  return scores
    .map((score, index) => {
      const point = polarPoint(center, (score.score / 100) * radius, index, scores.length);
      return `${point.x},${point.y}`;
    })
    .join(" ");
}

function splitLabel(label: string) {
  if (label === "Emotional Sensitivity") return ["Emotional", "Sensitivity"];
  return [label];
}

export default function BigFiveRadarChart({ scores }: { scores: DimensionScore[] }) {
  const center = 180;
  const radius = 94;
  const rings = [20, 40, 60, 80, 100];
  const polygonPoints = pointsFor(scores, center, radius);

  return (
    <section className="rounded-[1.5rem] border border-ink/10 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h3 className="text-xl font-black">Big Five Radar</h3>
          <p className="mt-1 text-sm leading-6 text-ink/58">A five-trait view of your current profile.</p>
        </div>
        <span className="w-fit rounded-full bg-mist px-3 py-1 text-xs font-bold text-ink/58">0-100 profile</span>
      </div>

      <div className="mt-4 overflow-hidden rounded-[1.25rem] bg-mist/70">
        <svg aria-label="Big Five radar chart" className="h-auto w-full" role="img" viewBox="0 0 360 336">
          <defs>
            <linearGradient id="big-five-radar-fill" x1="0%" x2="100%" y1="0%" y2="100%">
              <stop offset="0%" stopColor="#ff7a70" stopOpacity="0.42" />
              <stop offset="100%" stopColor="#f5cf65" stopOpacity="0.5" />
            </linearGradient>
          </defs>

          {rings.map((ring) => {
            const ringRadius = (ring / 100) * radius;
            const ringPoints = scores
              .map((_, index) => {
                const point = polarPoint(center, ringRadius, index, scores.length);
                return `${point.x},${point.y}`;
              })
              .join(" ");

            return (
              <polygon
                fill="none"
                key={ring}
                points={ringPoints}
                stroke="rgba(23,32,42,0.12)"
                strokeWidth="1"
              />
            );
          })}

          {scores.map((score, index) => {
            const outer = polarPoint(center, radius, index, scores.length);
            return (
              <line
                key={score.id}
                stroke="rgba(23,32,42,0.14)"
                strokeWidth="1"
                x1={center}
                x2={outer.x}
                y1={center}
                y2={outer.y}
              />
            );
          })}

          <polygon fill="url(#big-five-radar-fill)" points={polygonPoints} stroke="#ff7a70" strokeWidth="3" />

          {scores.map((score, index) => {
            const point = polarPoint(center, (score.score / 100) * radius, index, scores.length);
            return (
              <g key={score.id}>
                <circle cx={point.x} cy={point.y} fill="#17202a" r="4" />
                <circle cx={point.x} cy={point.y} fill="none" r="8" stroke="rgba(255,122,112,0.35)" strokeWidth="3" />
              </g>
            );
          })}

          {scores.map((score, index) => {
            const labelPoint = polarPoint(center, 130, index, scores.length);
            const anchor = Math.abs(labelPoint.x - center) < 12 ? "middle" : labelPoint.x > center ? "start" : "end";
            const lines = splitLabel(score.label);

            return (
              <text
                fill="#17202a"
                fontSize="12"
                fontWeight="800"
                key={score.id}
                textAnchor={anchor}
                x={labelPoint.x}
                y={labelPoint.y}
              >
                {lines.map((line, lineIndex) => (
                  <tspan dy={lineIndex === 0 ? 0 : 14} key={line} x={labelPoint.x}>
                    {line}
                  </tspan>
                ))}
                <tspan dy="16" fill="rgba(23,32,42,0.58)" fontSize="11" fontWeight="700" x={labelPoint.x}>
                  {score.score}
                </tspan>
              </text>
            );
          })}
        </svg>
      </div>
    </section>
  );
}
